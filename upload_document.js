...
function upload() {
  if (index < wilayah.length) {
    ...
    const fileNameUpload = `${nama_provinsi}_${nama_kabkota}_${nama_kecamatan}_${nama_kelurahan}_DTT.pdf`;
    ...
    if (!fs.existsSync(filePathUpload)) {
      ...
      if (rekapData.file_bast_pbp_verification_status === "verified") {
        ...
        form.append("document_type", getDocumentTypeUpload("DTT"));
        ...
        const req = https.request(url, options, (res) => {
          ...
          res.on("end", () => {
            ...
            if (jsonResponse.message === "File baru berhasil ditambahkan") {
              ...
              "Dokumen dengan tipe file_bast_pbp sudah diverifikasi."
            ) {
              index++;
              upload();
            }
