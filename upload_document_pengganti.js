...
function upload() {
  if (index < wilayah.length) {
    ...
    const fileNameUpload = `${nama_provinsi}_${nama_kabkota}_${nama_kecamatan}_${nama_kelurahan}_PENGGANTI.pdf`;
    ...
    if (!fs.existsSync(filePathUpload)) {
      ...
      if (
        rekapData.file_bast_pengganti_verification_status === "verified" &&
        rekapData.file_bast_pengganti_catatan === ""
      ) {
        console.log(
        ...
        console.log(
          `Mengupload dokumen untuk ${nama_kecamatan}_${nama_kelurahan}`
        );
        const form = new FormData();

        form.append("filename", `${no_bast}`);
        form.append("document_type", getDocumentTypeUpload("PENGGANTI"));
        ...
        const req = https.request(url, options, (res) => {
          ...
          res.on("end", () => {
            ...
            if (jsonResponse.message === "File baru berhasil ditambahkan") {
              ...
              "Dokumen dengan tipe file_bast_pengganti sudah diverifikasi."
            ) {
              console.log(
                `Dokumen untuk ${nama_kecamatan}_${nama_kelurahan} sudah diverifikasi sebelumnya`
              );
              index++;
              upload();
            }
